...
### Maintained Kanban lifecycle contract

The local health controller (`hermes_cli/kanban_health.py`) runs on dispatcher
ticks. Preserve affirmed typed atomic human gates, occurrence-bound scheduled
holds, ready-queue reason codes, and the deterministic sentinel. Legacy untyped
holds are reported, never automatically resumed; dependency/wake holds resume
exactly once while external/physical/roadmap holds stay parked. Full operations
contract: `docs/kanban/health-control-loop.md`.

Updates must retain the lifecycle capabilities certified by
`hermes_cli/kanban_capabilities.py`. The candidate cannot certify itself by
removing a required manifest entry: requirements come from the running install.
