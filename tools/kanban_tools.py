...
def _handle_show(args: dict, **kw) -> str:
    ...
    with _board(args.get("board")) as (kb, conn):
        ...
        from hermes_cli import kanban_health as kh
        return json.dumps({
            "task": kh.project_task_serialization(conn, task, _fields(task, _TASK_FIELDS)),
...
def _handle_list(args: dict, **kw) -> str:
    ...
    with _board(args.get("board")) as (kb, conn):
        ...
        rows = kb.list_tasks(
            ...
            limit=None if args.get("status") == "blocked" else limit + 1)
        # Project before limiting: machine/stale blocks must not hide affirmed gates.
...
def _inherited_parent_scope(kb, conn, parents) -> tuple[dict, Optional[str]]:
    ...
    if not self_tid or not _is_dispatcher_owned_worker():
...
def _handle_create(args: dict, **kw) -> str:
    ...
    board_error = _board_authority_error(args.get("board"))
    _check(not board_error, f"kanban_create: {board_error}")
    ...
    workspace_kind = workspace_kind if workspace_kind is not None else "scratch"
    ...
    if self_tid:
        # Explicitly pin the connection: ambient DB/current-board overrides are not authority.
        board = os.environ["HERMES_KANBAN_BOARD"]
    with _board(board) as (kb, conn):
        from tools.async_delegation import _current_origin_session_id
        session_id = args.get("session_id")
        ambient_session_id = _current_origin_session_id() or os.environ.get("HERMES_SESSION_ID")
        ...
        create_kwargs = dict(
            title=str(title).strip(), body=args.get("body"), assignee=str(assignee),
            ...
            created_by=(_scope.get("created_by") if _scope else os.environ.get("HERMES_PROFILE") or "worker"),
            session_id=session_id)
        # Validate under the same write transaction as insertion to close reclaim/scope races.
        with kb.write_txn(conn):
            fresh_scope, fresh_error = _inherited_parent_scope(kb, conn, parents)
            _check(not fresh_error, fresh_error or "")
            _check(fresh_scope == _scope, "authoritative parent scope changed during creation")
