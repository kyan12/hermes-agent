...
- Registers tool-callable supervisor surfaces (`supervisor_control`, `worker_callback`) so parent agents and dispatched workers can use the same durable control plane instead of private Python-only helpers.
...
- Defines a portable JSON worker registry contract with capability/risk/cadence metadata (`default_worker_registry`, `normalize_worker_registry`, `plan_worker_dispatch`, `assign_worker_to_task`, `assign_stored_worker_to_task`, `dispatch_worker_task`, `dispatch_stored_worker_task`). The registry avoids Hermes-only classes, secrets, or callables so the same envelope can be copied into other agent systems and implemented with their native dispatch transport.
- Dispatches ready workers through an injected native transport adapter with only the sanitized handoff envelope; successful dispatch moves the task to `waiting_agent` and records bounded dispatch audit metadata without accepting inline worker results.
...
`append_worker_callback` is authenticated when called through the durable control plane. `assign_worker_to_task()` / `assign_stored_worker_to_task()` attach a `handoff.callback` envelope with an opaque worker task ref, action name, token, and callback-id requirement; the persisted task assignment stores only `callback_auth.token_hash` plus bounded used callback IDs. Workers must submit `task_id`, `callback_token`, and a unique `callback_id`/`nonce` via `supervisor_control("append_worker_callback", ...)`, the thin `worker_callback(payload=...)` Python wrapper, or the registered `worker_callback` tool. Bad or missing tokens return a minimal `unauthorized` response without saving or rendering the dashboard, and duplicate callback IDs return `duplicate` without appending another callback or advancing controller audit sequence.

The plugin registers two tools in the `supervisor` toolset: `supervisor_control` for allowlisted control-plane actions and `worker_callback` for worker result/blocker submission. Tool handlers return JSON, use the same lock-scoped control plane as Python callers, and keep callback tokens out of persisted state. Worker dispatch prompts should enable the `supervisor` toolset for child agents that need to report back directly.
...
`dispatch_worker_task(data, task_id, dispatcher=..., required_capabilities=...)` assigns a ready worker and immediately calls the injected dispatcher while the callback token is still only in memory. The dispatcher receives exactly `{"schema_version", "worker_id", "handoff"}`; the `handoff` is the same sanitized worker envelope used by planning plus the token-bearing `callback` block. The task store persists only dispatch audit fields (`worker_dispatches`, `worker_dispatch_status`, `worker_dispatched_at`) and the callback token hash already stored in `worker_assignment.callback_auth`.

`dispatch_worker_assignment(data, task_id, dispatcher=..., assignment=plan)` can dispatch a just-returned ready assignment. If called later with only the persisted `worker_assignment`, it fails closed as `missing_callback_token` rather than trying to recover or persist a raw token. `dispatch_stored_worker_task(...)` is the durable helper: it loads the store, assigns and dispatches in one lock-scoped pass, saves audit state, and returns `audit_persisted`.

Dispatchers may return `success` plus a `dispatch_id`/`id`; inline worker summaries or statuses are deliberately ignored. Workers must report all results and blockers through `worker_callback(payload=...)` / `supervisor_control("append_worker_callback", ...)` with the authenticated callback envelope.
