...
def get_task(
    ...
    with _board_conn(board) as (board, conn):
        ...
        task_d = _task_dict(conn, task, latest_summary=kanban_db.latest_summary(conn, task_id))
        ...
        return {
            ...
            "links": links,
            "child_results": [
                kh.project_task_serialization(conn, c, {"id": c.id, "title": c.title, "status": c.status, "latest_summary": child_summaries.get(c.id), "result": c.result})
                for c in children],
...
def _apply_status(conn, task_id: str, s: str, p, unknown_detail: str) -> bool:
    ...
    if s == "scheduled":
        raise _StatusRejected("scheduled requires POST /tasks/{id}/hold with a typed hold kind")
