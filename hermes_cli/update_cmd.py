...
from hermes_cli.update_cmd_capabilities import (
    _installed_capability_manifest_present,
    _preflight_git_capability_candidate,
    _run_capability_canary,
)
...
def _cmd_update_impl(args, gateway_mode: bool):
    ...
    try:
        ...
        # Validate the exact maintained candidate before replacing live code.
        if not _preflight_git_capability_candidate(
            git_cmd, _m().PROJECT_ROOT, branch, merge_in_place=_plan.in_place_update,
        ):
            _m()._resume_windows_gateways_after_update(_windows_gateway_resume)
            sys.exit(1)
