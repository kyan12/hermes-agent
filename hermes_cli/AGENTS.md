...
- **Maintained lineage**: explicit `--branch` → `update.branch` → `main`.
  `update_cmd_capabilities.py` certifies the exact candidate before activation,
  using the running installation's required lifecycle manifest. For in-place
  updates, certify HEAD merged with upstream, not bare upstream. Missing or
  unprobeable capabilities fail closed. Preserve this gate when refactoring.
