...
def _fire_dispatch_tick_hook(
    ...
    try:
        ...
        elif not any(getattr(result, f) for f in _TICK_ACTIVITY_FIELDS) and not (
            result.health_resumed or result.health_needs_attention
        ):
            outcome = "idle"
        invoke_hook(
...
class Task:
    ...
    # Occurrence-bound typed hold/gate authority; validated by kanban_health.
    ...
    def from_row(cls, row: sqlite3.Row) -> "Task":
        ...
        return cls(
            ...
            **{col: g(col) or None for col in _TASK_EMPTY_IS_NULL_COLUMNS},
            ...
            hold_kind=g("hold_kind") or None,
            hold_wake_at=int(g("hold_wake_at")) if g("hold_wake_at") is not None else None,
            gate_evidence=g("gate_evidence") or None,
        )
...
def block_task(
    ...
    """Machine/worker requests route to ``todo`` (dependency) or ``triage``.
    Only kanban_health.affirm_human_gate may create an occurrence-bound human
    ``blocked`` gate. Preserve recurrence history and auditable events."""
    ...
    with write_txn(conn):
        ...
        run_id = _end_or_synthesize_run(
            conn, task_id, outcome="blocked", status="triage" if new_status == "triage" else "blocked",
            summary=reason, synthesize=bool(reason),
        )
        _append_event(conn, task_id, event_kind, payload, run_id=run_id)
        if kind != "dependency" and payload["recurrences"] >= BLOCK_RECURRENCE_LIMIT:
            _append_event(conn, task_id, "block_loop_detected", {
                "kind": kind, "recurrences": payload["recurrences"], "reason": reason,
            }, run_id=run_id)
        if reason and author:
            add_comment(conn, task_id, author, f"BLOCKED: {reason}")
    # Human-gate notifications belong exclusively to affirm_human_gate,
    # after its evidence transaction commits. A machine/dependency request
    # must not page an operator as though human authority were affirmed.
    return True
...
def _route_block(
    ...
    """Route dependency waits to todo and all unaffirmed requests to triage."""
    ...
    payload["requires_affirmation"] = kind in ("needs_input", "capability")
    return "triage", "automation_recovery_requested", set_sql, (kind, recurrences), payload
...
def schedule_task(
    ...
    until ``unblock_task`` re-gates it. Clear typed authority from the prior
    occurrence: this untyped schedule must remain legacy_untyped until classified."""
    with write_txn(conn):
