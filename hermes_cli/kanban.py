...
def _is_delegated_child_cli_mutation(args: argparse.Namespace) -> bool:
    ...
    if action == "boards":
        if (getattr(args, "boards_action", None) or "list") not in _DELEGATED_CHILD_DENIED_BOARD_ACTIONS:
            return False
    elif action in {"board-health", "scheduled-wake"}:
...
def _cmd_create(args: argparse.Namespace) -> int:
    ...
    with kbc.connect_closing() as conn:
        ...
        payload = _task_to_dict(task, conn)
    if getattr(args, "json", False):
        _print_json(payload)
    else:
        print(f"Created {task_id}  ({payload['status']}, assignee={payload.get('assignee') or '-'})")
...
def _cmd_list(args: argparse.Namespace) -> int:
    ...
    with kbc.connect_closing() as conn:
        ...
        projections = {t.id: kh.classify_block(conn, t) for t in tasks if t.status == "blocked"}
        if args.status == "blocked":
            tasks = [t for t in tasks if projections[t.id].visible]
        serialized = [_task_to_dict(t, conn) for t in tasks]
    if _json_out(args, serialized):
        return 0
...
def _cmd_show(args: argparse.Namespace) -> int:
    ...
    if want_json:
        _print_json({
            "task": task_payload, "latest_summary": latest_summary, "parents": parents, "children": children,
    ...
    field("status", task_payload["status"])
...
_HANDLERS = {
    ...
    "board-health": _cmd_board_health, "scheduled-wake": _cmd_scheduled_wake,
