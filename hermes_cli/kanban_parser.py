...
_SPECS = [
    ...
    _cmd("block", [
        ...
        _bulk_ids("block"),
        _arg("--action", help="One atomic action; required for needs_input/capability human gates."),
        _arg("--evidence-type", default="human_decision", choices=sorted(kh.VALID_EVIDENCE_TYPES)),
        _arg("--affirmed-by", help="Verified human principal bound to this install via kanban.operator_principal."),
    ...
    _cmd("schedule", [
        ...
        _bulk_ids("schedule"),
        _arg("--kind", required=True, choices=sorted(kh.VALID_HOLD_KINDS),
             help="Typed hold. dependency/wake may auto-resume; intentional holds require typed evidence."),
        _arg("--wake-at", metavar="<epoch|+SECONDS>", help="Required for --kind wake."),
    ], help="Park one or more tasks in Scheduled (waiting on time, not human input)"),
    _cmd("board-health", [
        _json_flag(),
        _arg("--all-boards", action="store_true"),
        _arg("--reconcile", action="store_true"),
        _arg("--ready-queue", action="store_true"),
    ...
    ], help="Run a deterministic sentinel sweep over every board"),
    _cmd("unblock", [
