...
`triage` 仅用于主动录入、规格完善和操作员指定的待整理工作。运行失败或重复阻塞不会进入此列。
`needs_input`、`capability`、`transient` 和未指定类型的阻塞始终保持 `blocked`；
重复达到阈值时仍发出 `block_loop_detected` 事件，保留次数、通知去重和来源会话唤醒。
调度器不会自动恢复这些任务。`dependency` 阻塞仍进入 `todo`，父任务完成后自动推进。

旧版本留在 triage 的重复阻塞任务可由操作员修复：

```bash
hermes kanban --board <board> repair-block-loop <id> --actor <operator> --reason "恢复人工处理状态"
```

此操作原子地恢复 `blocked` 并写入审计事件，保留阻塞原因、次数及原升级事件编号。
不会经过 Ready、生成运行记录或启动工作；重复修复不产生新事件。
存在运行所有权、未结束运行、证据不一致或处于其他阶段的任务会被拒绝。
修复事件使用旧读取器也认识的 `blocked` 类型。后续运行应统一升级写入器和调度器，
因为旧写入器仍可能把新阻塞送入 triage，旧调度器也不认识独立升级事件的粘性阻塞语义。
尚未修复的旧卡片仍可能被自动规格完善或分解。
