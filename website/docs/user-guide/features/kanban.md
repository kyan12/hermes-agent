...
Every committed trigger event immediately creates an ordinary Kanban task with the deterministic key `kanban-reconcile:<board>:<source-task>:<event-id>`. There is no polling queue or sidecar state. The reconciliation worker must complete with one machine-readable outcome: `cleared/resumed`, `cleared/completed`, `continuation_created`, `dependency_wait`, `backoff_scheduled`, `genuine_human_gate`, or `reconciliation_failed`. `cleared/completed` is restricted to a live kernel-created reconciler accepting a completed direct continuation; it atomically preserves that handoff, closes the source, and releases downstream work. Only `genuine_human_gate` (with one exact requested action) wakes or notifies the origin user; all other outcomes remain automation-owned and auditable in `task_events`.
